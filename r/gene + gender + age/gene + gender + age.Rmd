...
lr_fit3 = update(lr_fit2, . ~ . - NM_016021)
summary(lr_fit3)

extract_eq(lr_fit3, use_coefs = TRUE)
```
...
for (i in 1:n_sim) {
  ...
  for (j in 1:cvK) {
    ...
    current_lr_fit = glm(outcome ~ . - sex - NM_016021, data = train,
                         family = binomial(link = "logit"))
    predicted_probs = predict(current_lr_fit, X_test, 
                              type = "response") # Predicting probability of stable
    predictions = ifelse(round(predicted_probs) == 1, 

...
df_knn = cbind(Accuracy = cv_accuracy_knn, Classifier = "6-Nearest Neighbor")
# df_dt = cbind(Accuracy = cv_accuracy_dt, Classifier = "Decision Trees")

df_classifier_cv = data.frame(rbind(df_lr, df_rf, df_svm, df_nb, df_knn))
...
final_model = naive_bayes(outcome ~ ., train)
...
table(Predictions = predictions1, Class = y_test1) %>% kable(caption = "Confusion table when testing on GSE51675")

performance1 = data.frame(Accuracy = (3+9)/(3+0+6+9), Sensitivity = (3)/(3+6), Specificity = (9)/(0+9))
performance1 %>% kable(caption = "Performance evaluation on GSE51675", digits = 2)

predictions3 = predict(final_model, X_test3)
table(Predictions = predictions3, Class = y_test3) %>% kable(caption = "Confusion table when testing on GSE46474")

performance3 = data.frame(Accuracy = (7+19)/(7+1+13+19), Sensitivity = (7)/(7+13), Specificity = (19)/(1+19))

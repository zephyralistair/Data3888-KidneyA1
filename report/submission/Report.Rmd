...
author: 'KidneyA1: Qinlin(Alistair) Gu, Danning Sun, Hangyi Wu, Xuening Ni, Yuye(Giri) Zhou'
date: "2022/5/30"
output: 
  rmdformats::downcute:
    fig_caption: yes
...
Eventually, we built models that contain age, gender, and the two most variable gene expression values ([SLC22A14](https://www.genecards.org/cgi-bin/carddisp.pl?gene=SLC22A14), [TAP2](https://www.genecards.org/cgi-bin/carddisp.pl?gene=TAP2)) as variables, using multiple supervised machine learning algorithms.

Note to allow the model to be tested on the other two independent datasets, these two most variable genes (SLC22A14, TAP2) were selected from the intersection of the three datasets. [Appendix Fig.6]
...
We then deployed our final classifier using an R package Shiny. We designed the app to make our target audience of doctors easy to predict and interpret the patient's kidney graft outcome. We divided the app into three parts.
...
The third part is the predictor. The doctor can obtain the patient's kidney graft outcome by inputting the patient's gender, age, and two gene expression values, "TAP2" and "SLC22A14".
...
Our collaboration platform was [GitHub](https://github.com/zephyralistair/Data3888-KidneyA1). Our product was deployed at [shinyapps](https://zephyralistair.shinyapps.io/DATA3888_KidneyA1/).
